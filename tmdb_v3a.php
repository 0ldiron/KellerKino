...
###############################################################################################################
	...
	*/
		public function movieCrew($idMovie)
		{
			...
			$tm_info = $this->movieInfo($idMovie,'casts');

			foreach ($tm_info['crew'] as $crew)
			{
				if (0 == strcasecmp($crew['job'], 'Director'))
				{
					if (strlen($ret_dir) == 0) $ret_dir = $crew['name'];
					else $ret_dir .= ', '.$crew['name']; 	
				}
				if (0 == strcasecmp($crew['job'], 'Writer'))
				{
					if (strlen($ret_wri) == 0) $ret_wri = $crew['name'];
					else $ret_wri .= ', '.$crew['name']; 	
				}
			}

			foreach ($tm_info['cast'] as $cast)
			{
				$i = strlen($ret_cast);
				if ($i > 80) break;
				if ($i == 0) $ret_cast = $cast['name'];
				else $ret_cast .= ', '.$cast['name']; 	
			}
		
			return array('cast'=>$ret_cast, 'Director'=>$ret_dir, 'Writer'=>$ret_wri);
		}//end of movieCrew
